...
class EdgePlacement(Placement):
    def initial_allocation(self, sim, app_name):
        value = {"mytag": "edge"}
        value_2 = {"mytag": "client"}
        id_cluster = sim.topology.find_IDs(value)
        id_cluster_2 = sim.topology.find_IDs(value_2)
        app = sim.apps[app_name]
        services = app.services

        for module in services:
            if module in self.scaleServices:
                if module is not "Client":
                    print(module+"edge")
                    ...
                    print(module + "client")
                    for rep in range(0, self.scaleServices[module]):
                        idDES = sim.deploy_module(app_name, module, services[module], id_cluster_2)
...
def create_application():
    ...
    a.set_modules([{"Sensor": {"Type": Application.TYPE_SOURCE}},
                   ...
                   {"ServiceC": {"RAM": 10, "Type": Application.TYPE_MODULE}},
                   ])
    """
    Messages among MODULES (AppEdge in iFogSim)
    """
    m_sense = Message("M.sense", "Sensor", "Client", instructions=20 * 10 ** 6, bytes=1000)
    m_b = Message("M.B", "Client", "ServiceB", instructions=30 * 10 ** 6, bytes=500)
    m_c = Message("M.C", "Client", "ServiceC", instructions=30 * 10 ** 6, bytes=500)
    m_b2 = Message("M.B2", "ServiceB", "Client", instructions=30 * 10 ** 6, bytes=500)
    m_c2 = Message("M.C2", "ServiceC", "Client", instructions=30 * 10 ** 6, bytes=500)

    """
    Defining which messages will be dynamically generated # the generation is controlled by Population algorithm
    """
    a.add_source_messages(m_sense)

    """
    MODULES/SERVICES: Definition of Generators and Consumers (AppEdges and TupleMappings in iFogSim)
    """
    # MODULE SERVICES
    a.add_service_module("Client", m_sense, m_b, fractional_selectivity, threshold=1.0)
    a.add_service_module("Client", m_sense, m_c, fractional_selectivity, threshold=1.0)
    a.add_service_module("ServiceB", m_b, m_b2, fractional_selectivity, threshold=1.0)
    a.add_service_module("ServiceC", m_c, m_c2, fractional_selectivity, threshold=1.0)
...
def create_json_topology():
    ...
    edge_device_1 = {"id": 1, "model": "edge_server", "mytag": "edge", "IPT": 5000 * 10 ** 6, "RAM": 40000, "COST": 3,
                     "WATT": 20.0}

    sensor_dev = {"id": 2, "model": "sensor_device", "IPT": 100 * 10 ** 6, "RAM": 4000, "COST": 3, "WATT": 40.0}
    client = {"id": 3, "model": "client", "mytag": "client", "IPT": 100 * 10 ** 6, "RAM": 4000, "COST": 3, "WATT": 40.0}

    link1 = {"s": 2, "d": 3, "BW": 1, "PR": 10}
    link2 = {"s": 3, "d": 1, "BW": 1, "PR": 1}

    topology_json["entity"].append(edge_device_1)
    topology_json["entity"].append(sensor_dev)
    topology_json["entity"].append(client)
...
def main(simulated_time):
    ...
    placement.scaleService({"Client": 1, "ServiceB": 1, "ServiceC": 1})
    ...
    pop.set_src_control(
        {"model": "sensor_device", "number": 1, "message": app.get_message("M.sense"), "distribution": dDistribution})
