...
from yafs.entities_mert import Entity, Base_Station, Edge_Server, Sink, Source, Mobile_Device
...
def up_rate_evaluation_fnc(simulated_time, uplink_rate):
    def add_drone(uplink_rate, user_id, attached_bs_id):
        ...
        camera_user = Source(id=entity_id_counter(), model="camera", ipt=100 * (10 ** 6), ram=4000000,
        ...
        imu_user = Source(id=entity_id_counter(), model="imu", ipt=100 * (10 ** 9), ram=4000000,
                          cost=3, power_consumption=40.0, user=user_id)
        # imu_user = {"id": entity_id_counter(), "model": "imu", "IPT": 100 * (10 ** 9), "RAM": 400000, "COST": 3,
        #             "WATT": 40.0, "user": user_id}

        id_actuator = ID_COUNTER_ENTITY
        actuator_user = Sink(id=entity_id_counter(), model="actuator_device", ipt=1 * (10 ** 6), ram=4000000,
                               cost=3, power_consumption=40.0, user=user_id)
        ...
        drone_user = Mobile_Device(id=entity_id_counter(), model="drone_user", ipt=DRONE_CPU, ram=4000000,
        ...
        links.append(Link(source=id_camera, destination=id_drone, bandwidth=1000, latency=0))

        # links.append({"s": id_imu, "d": id_drone, "BW": 1000, "PR": 0})
        links.append(Link(source=id_imu, destination=id_drone, bandwidth=1000, latency=0))

        # links.append({"s": id_drone, "d": id_actuator, "BW": 1000, "PR": 0})
        links.append(Link(source=id_drone, destination=id_actuator, bandwidth=1000, latency=0))

        # links.append({"s": id_drone, "d": attached_bs_id, "BW": uplink_rate, "PR": 0})
        links.append(Link(source=id_drone, destination=attached_bs_id, bandwidth=uplink_rate, latency=0))

        # links.append({"s": attached_bs_id, "d": id_drone, "BW": uplink_rate, "PR": 0})
        links.append(Link(source=attached_bs_id, destination=id_drone, bandwidth=uplink_rate, latency=0))
    ...
    def add_bs_and_drones(attached_edge_server_ids):
        ...
        for i in range(len(attached_edge_server_ids)):
            ...
            links.append(Link(source=id_bs, destination=attached_edge_server_ids[i], bandwidth=1000, latency=0))

            # links.append({"s": attached_edge_server_ids[i], "d": id_bs, "BW": 1000, "PR": 0})
            links.append(Link(source=attached_edge_server_ids[i], destination=id_bs, bandwidth=1000, latency=0))
