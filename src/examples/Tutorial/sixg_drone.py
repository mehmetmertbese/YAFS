...
FEATURE_SIZE = 12
UPLINK_RATE = (10 / 8) * (10 ** -3)
UPLINK_RATE_ARRAY = [x * ((10 / 8) * (10 ** -3)) for x in range(1, 10)]
...
def create_application():
    ...
    m_extracted_features = Message("Extracted Features", "Feature_Extraction", "MSCKF_Update",
                                   instructions=BACK_END_CMPT_LOAD, bytes=NUMBER_OF_FEATURES * FEATURE_SIZE + TIMESTAMP)
...
def create_json_topology():
    ...
    edge_device = {"id": 3, "model": "edge_server", "mytag": "edge",
                   ...
                   "WATT": 20.0}
    ground_station = {"id": 4, "model": "ground_station", "mytag": "ground_station", "IPT": 20 * (10 ** 9), "RAM": 4000,
                      "COST": 3, "WATT": 40.0}
...
def main(simulated_time):
    ...
    dDistribution = deterministicDistributionStartPoint(name="Deterministic", time=3000, start=10)
    dDistribution_2 = deterministicDistributionStartPoint(name="Deterministic", time=3000, start=10)
    pop.set_src_control(
    ...
    tim_out_read(folder_results + "sim_trace_full_r4_n20_faster_connection.csv", UPLINK_RATE)
