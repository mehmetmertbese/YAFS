...
from examples.Tutorial.csv_reader import tim_out_read,tim_out_read_ratio
import pandas as pd
...
CMPT_PWR_RATIO_ARRAY = [x for x in range(1,20)]
TIMESTAMP = 4
UDP_OVERHEAD = 24
...
FEATURE_SIZE = 12
UPLINK_RATE = (800 / 8) * (10 ** -3)
UPLINK_RATE_ARRAY = [(x * ((10 / 8) * (10 ** -3))) for x in range(1, 100)]
DOWNLINK_RATE = (800/8) * (10 ** -3)
folder_results = Path("results/")
folder_results.mkdir(parents=True, exist_ok=True)
folder_results = str(folder_results) + "/"
CSV_FILE = folder_results + 'sim_trace_full_edge_cmpt_ratio_fast_uplink.csv'

from yafs.placement import Placement


# @profile
def up_rate_evaluation_fnc(simulated_time, ratio):
    ...
    def create_json_topology(ratio):
        """
           TOPOLOGY DEFINITION
        ...
        edge_device = {"id": 3, "model": "edge_server", "mytag": "edge",
                       "IPT": DRONE_CPU * ratio, "RAM": 40000, "COST": 3,
                       "WATT": 20.0}
    ...
    """
    t = Topology()
    t_json = create_json_topology(ratio)
    t.load(t_json)
    ...
    s = Sim(t, default_results_path=folder_results + "sim_trace_full_edge_cmpt_ratio_fast_uplink")
    ...
    tim_out_read_ratio(CSV_FILE, ratio)

    # s.draw_allocated_topology() # for debugging


def main():
    for i in range(len(CMPT_PWR_RATIO_ARRAY)):
        up_rate_evaluation_fnc(simulated_time=70000, ratio=CMPT_PWR_RATIO_ARRAY[i])
